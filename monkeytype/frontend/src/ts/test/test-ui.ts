...
const NEWLINE_ICON_HTML =
  '<i class="fas fa-level-down-alt fa-rotate-90 fa-fw"></i>';

type SpecialCharInfo = {
  html: string;
  className: "tabChar" | "nlChar";
  isNewline: boolean;
};
...
function getSpecialCharInfo(char: string): SpecialCharInfo | null {
  if (char === "\t") {
    return {
      html: TAB_ICON_HTML,
      className: "tabChar",
...
function buildSpecialLetterHtml(
  info: SpecialCharInfo,
  extraClasses?: string,
  style?: string,
): string {
  const classNames = [info.className, extraClasses].filter(Boolean).join(" ");
  const styleAttr = style && style.length > 0 ? ` style="${style}"` : "";
  return `<letter class='${classNames}'${styleAttr}>${info.html}</letter>`;
}
...
function buildWordHTML(word: string, wordIndex: number): string {
  ...
  for (const char of chars) {
    if (funbox) {
      ...
      const special = getSpecialCharInfo(char);
      if (special) {
        newlineafter ||= special.isNewline;
        retval += buildSpecialLetterHtml(special);
        continue;
      }
    ...
    async () => {
      ...
      if (Config.mode === "zen") {
        for (const char of input) {
          ...
          if (special) {
            newlineafter ||= special.isNewline;
            ret += buildSpecialLetterHtml(special, "correct", "opacity: 0");
            continue;
          }
        ...
        for (let i = 0; i < inputChars.length; i++) {
          ...
          if (funbox) {
            ...
            const special = getSpecialCharInfo(currentLetter);
            if (special) {
              tabChar = special.className === "tabChar" ? "tabChar" : "";
              nlChar = special.className === "nlChar" ? "nlChar" : "";
              currentLetter = special.html;
            }
          }
          ...
          if (charCorrect) {
            ...
            if (letter === " ") {
              ...
              const special = getSpecialCharInfo(letter);
              if (special) {
                letter = special.html;
              }
            }
        ...
        for (
          ...
          if (funbox?.functions?.getWordHtml) {
            ...
            const special = getSpecialCharInfo(currentLetter as string);
            if (special) {
              ret += buildSpecialLetterHtml(special);
              continue;
            }
...
export function updatePremid(): void {
  const mode2 = Misc.getMode2(
    Config,
    TestWords.currentQuote as unknown as { id: number } | null,
  );
  let fbtext = "";
...
function resetLiveIndicators(): void {
...
export function updateVerseReference(reference: string | null): void {
  const referenceEl = qs(".pageTest #verseReference");
  if (!referenceEl) return;
  if (!reference) {
    referenceEl.addClass("hidden");
    referenceEl.setText("-");
    return;
  }
  referenceEl.removeClass("hidden");
  referenceEl.setText(reference);
}
...
ConfigEvent.subscribe(({ key, newValue }) => {
  ...
  if (["tapeMode", "tapeMargin"].includes(keyString)) {
