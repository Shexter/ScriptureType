...
export function setNotSignedInUidAndHash(uid: string): void {
  ...
  delete (notSignedInLastResult as { hash?: string }).hash;
...
function getQuickRestartDisabledMessage(): string {
  if (Config.quickRestart === "tab") {
    return "Press shift + tab or use your mouse to confirm.";
  }
  if (Config.quickRestart === "esc") {
    return "Press shift + escape or use your mouse to confirm.";
  }
  if (Config.quickRestart === "enter") {
    return "Press shift + enter or use your mouse to confirm.";
  }
  return "Use your mouse to confirm.";
}

function shouldBlockQuickRestart(options: RestartOptions): boolean {
  if (getActivePage() !== "test") return false;
  if (ManualRestart.get()) return false;
  ...
  Notifications.add(
    `Quick restart disabled in long tests. ${getQuickRestartDisabledMessage()}`,
    0,
    {
      duration: 4,
      important: true,
...
function recordIncompleteTest(now: number): void {
  const testSeconds = TestStats.calculateTestSeconds(now);
...
export function restart(options = {} as RestartOptions): void {
  ...
  if (shouldBlockQuickRestart(options)) {
    return;
  }
  ...
  if (TestState.isActive) {
    ...
    if (TestState.savingEnabled) {
      ...
      recordIncompleteTest(performance.now());
    }
  }
...
async function init(): Promise<boolean> {
  ...
  TestUI.updateVerseReference(TestState.verseReference);
...
async function saveResult(
  ...
  delete (result as { hash?: string }).hash;
