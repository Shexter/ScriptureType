...
import { getChapterMeta, getChapterWords } from "./scripture-api";
import {
...
type LanguageObject = {
  ...
  ligatures?: boolean;
  noLazyMode?: boolean;
  additionalAccents?: LazyMode.Accents;
...
let isUsingScriptureSource = false;
...
export async function generateWords(
  ...
  if (Config.mode === "custom") {
    ...
    const minWords = limit > 0 ? limit : 100;
    const chapterWords = await getChapterWords(minWords);
    if (chapterWords.length === 0) {
      throw new WordGenError("Scripture API returned no words");
    }
    wordList = chapterWords;
    isUsingScriptureSource = true;
    TestState.setVerseReference(getChapterMeta()?.verseRange ?? null);
  }
...
export async function getNextWord(
  ...
  randomWord = await applyBritishEnglishToWord(randomWord, previousWord);
