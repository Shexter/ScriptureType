...
export async function replace(
  ...
  if (word.includes("-")) {
    ...
    if (Config.mode === "quote" && typeof rule !== "string") {
      const cleanedPreviousWord = previousWord
        .replace(/^[\W]+|[\W]+$/g, "")
        .toLowerCase();
      if (rule.exceptPreviousWords.includes(cleanedPreviousWord)) {
        return word;
      }
    }

    const [britishWord, _exceptions] =
      typeof rule === "string"
        ? [rule, []]
