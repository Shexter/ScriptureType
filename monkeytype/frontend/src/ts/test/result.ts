...
const resultChart =
  ChartController.result as unknown as Chart<"line" | "scatter", number[], string>;
...
function updateVerseReference(reference: string | null): void {
  if (reference === null || reference.length === 0) {
    qs("#result .stats .verseReference")?.hide();
    qs("#result .stats .verseReference .bottom")?.setText("-");
    return;
  }
  qs("#result .stats .verseReference")?.show();
  qs("#result .stats .verseReference .bottom")?.setText(reference);
}
...
export async function update(
  ...
  dontSave: boolean,
  verseReferenceText?: string | null,
): Promise<void> {
  resultAnnotation = [];
  ...
  updateVerseReference(verseReferenceText ?? null);
  ...
  ((resultChart.options as PluginChartOptions<"line" | "scatter">)
...
ConfigEvent.subscribe(async ({ key }) => {
  if (
    ["typingSpeedUnit", "startGraphsAtZero"].includes(String(key)) &&
