...
export function oxlintChecker(options: OxlintCheckerOptions = {}): Plugin {
  ...
  return {
    ...
    buildStart() {
      ...
      // #region agent log
      fetch(
        "http://127.0.0.1:7245/ingest/fcd1d3c3-1c32-4bb8-80de-88b7684b37b7",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "oxlint-checker.ts:buildStart",
            message: "Oxlint build hook entered",
            data: { plugin, typeAware },
            timestamp: Date.now(),
      ...
      try {
        ...
        fetch(
          "http://127.0.0.1:7245/ingest/fcd1d3c3-1c32-4bb8-80de-88b7684b37b7",
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              location: "oxlint-checker.ts:buildStart:afterExecSync",
              message: "Oxlint passed",
              data: { outputLength: output?.length ?? 0 },
              timestamp: Date.now(),
              sessionId: "debug-session",
              hypothesisId: "A",
            }),
        ...
        const stdout =
          error instanceof Error && "stdout" in error
            ? String((error as Error & { stdout?: string }).stdout ?? "").slice(
                0,
                500,
              )
            : "";
        fetch(
          "http://127.0.0.1:7245/ingest/fcd1d3c3-1c32-4bb8-80de-88b7684b37b7",
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              location: "oxlint-checker.ts:buildStart:catch",
              message: "Oxlint failed, build aborting",
              data: { errMsg, stdoutSnippet: stdout },
              timestamp: Date.now(),
              sessionId: "debug-session",
              hypothesisId: "A",
            }),
    ...
    closeBundle() {
      ...
      fetch(
        "http://127.0.0.1:7245/ingest/fcd1d3c3-1c32-4bb8-80de-88b7684b37b7",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "oxlint-checker.ts:closeBundle",
            message: "Vite closeBundle reached",
            data: {},
            timestamp: Date.now(),
            sessionId: "debug-session",
            hypothesisId: "E",
